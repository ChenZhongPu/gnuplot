...
The code can be found at `script/math.gp`. Here, we set the `term` (short for *terminal*) as `epslatex`, and the remaining options:
...
I also wrote a helper script (`script/tex2eps.sh`) to generate a standalone *.eps* figure from *.tex*. Suppose there are `example-inc.eps` and `example.tex` in the current folder.
