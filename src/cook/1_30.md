...
First, we prepare a `style` file which configs the *line*, *border*, *ticks*, *border* and *grid*, and then we can load it in our script:
...
By the way, one can also rename `style` to `.gnuplot` and put it in the home directory (it works on Linux and macOS). Then gnuplot would automatically load this file.